...
The key data structure defined in `r Biocpkg("pogos")` is
`DRTraceSet`.  An example is
```{r lkcc}
iric = iriCCLE()
iric 
```

The `plot` method reveals a basic motivation for the class.
```{r lkpl090, fig.height=2.5}
plot(iric)
```

Such profiling data are acquired from PharmacoDb using the `DRTraceSet`
constructor.  The arguments available are
```{r lkar}
args(DRTraceSet)
```
